...
class HaLinkDevice:
    ...
    async def _send_set_object(self, key: str, value: Any) -> None:
        """Object mode SET parancs ts_enable támogatással."""
        set_dict = {"value": value}

        # ts_enable = true esetén unix timestamp-et adunk hozzá
        if self.config.get("ts_enable", False):
            import time  # csak itt használjuk, nem kell globálisan importálni
            set_dict["ts"] = int(time.time())

        body = {"set": {key: set_dict}}
