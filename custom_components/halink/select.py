    ...
    def _apply_state(self, state: Dict[str, Any]) -> None:
        ...
        if raw is not None:
            ...
            if option in self._attr_options:
                ...
                # Invalid option érkezett → figyelmeztetés + mégis elfogadjuk (HA engedi)
                _LOG.warning(
                    f"Invalid option received for {self.entity_id}: '{option}' "
                    f"(available: {self._attr_options}) – still accepting it"
                )
                self._attr_current_option = option  # HA UI-ban piros lesz, de működik
